...
from app.models.analyzer import Cv, CvParseResult, JdParseResult, JobDescription, MatchResult
...
def create_cv(
    ...
    *,
    ...
    db.execute(update(Cv).where(Cv.account_id == account_id, Cv.is_latest.is_(True)).values(is_latest=False))
    version_number = int(
        db.scalar(select(func.coalesce(func.max(Cv.version_number), 0)).where(Cv.account_id == account_id)) or 0
    ) + 1
    cv = Cv(
    ...
    db.add(cv)
    db.flush()
    parsed = CvParseResult(cv_id=cv.cv_id, parse_status="Pending", parser_version=parser_version)
    db.add(parsed)
    db.commit()
    db.refresh(cv)
    db.refresh(parsed)
...
def get_cv_record(db: Session, cv_id: int, account_id: int) -> tuple[Cv, CvParseResult | None] | None:
    cv = get_owned_cv(db, cv_id, account_id)
    return (cv, get_latest_parse(db, cv.cv_id)) if cv else None


def list_cv_records(db: Session, account_id: int) -> list[tuple[Cv, CvParseResult | None]]:
    cvs = list(db.scalars(select(Cv).where(Cv.account_id == account_id).order_by(Cv.version_number.desc())))
...
def set_parse_success(db: Session, parsed: CvParseResult, *, text: str, payload: dict) -> None:
...
def delete_owned_cv(db: Session, cv: Cv) -> str:
    ...
    if was_latest and account_id is not None:
        next_latest = db.scalar(
            select(Cv).where(Cv.account_id == account_id).order_by(Cv.version_number.desc()).limit(1)
        )
        if next_latest:
...
def get_or_create_job_description(
    ...
    *,
    ...
    existing = db.execute(
        ...
        .join(JdParseResult, JdParseResult.job_description_id == JobDescription.job_description_id)
        .where(
...
def get_owned_match(db: Session, match_result_id: int, account_id: int) -> MatchResult | None:
    return db.scalar(
        select(MatchResult)
...
def get_match_title(db: Session, match: MatchResult) -> str:
    description = db.get(JobDescription, match.job_description_id) if match.job_description_id else None
