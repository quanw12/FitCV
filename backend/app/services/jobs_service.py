...
def list_managed(db: Session, account: Account) -> list[JobResponse]:
    return [_response(row) for row in jobs.managed_jobs(db, _company(account))]
...
def create(db: Session, account: Account, payload: JobCreate) -> JobResponse:
    ...
    return next(item for item in list_managed(db, account) if item.job_id == job.job_id)
...
def update(db: Session, account: Account, job_id: int, payload: JobUpdate) -> JobResponse:
    ...
    return next(item for item in list_managed(db, account) if item.job_id == job_id)
