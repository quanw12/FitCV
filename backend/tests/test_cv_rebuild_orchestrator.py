...
class TestRebuildCv:
    ...
    def test_normalizes_extracted_cv_before_render(self) -> None:
        cv = CVData(
            name="A",
            summary="Engineer.",
            skills=["Python ★★★★"],
        ...
        result = rebuild_cv(_MINIMAL_PDF, "cv.pdf", extractor=FakeExtractor(cv))
        assert result.preview_json.skills == ["Python"]
        assert result.preview_json.languages[0].proficiency == "Fluent"

    def test_mixed_extracted_cv_is_unified_with_polish(self) -> None:
        cv = CVData(
            name="Nguyen Van A",
        ...
        result = rebuild_cv(_MINIMAL_PDF, "cv.pdf", extractor=extractor)
        assert extractor.polish_calls == 1
        assert extractor.last_polish_language == "vi"
        assert result.preview_json.name == "Nguyen Van A"

    def test_consistent_extracted_cv_skips_extra_polish(self) -> None:
        cv = CVData(
            name="Nguyen Van A",
            summary="Backend engineer with 3 years of experience.",
        )
        extractor = FakeExtractor(cv)
        rebuild_cv(_MINIMAL_PDF, "cv.pdf", extractor=extractor)
        assert extractor.polish_calls == 0

    def test_vietnamese_name_survives_unify_polish(self) -> None:
        cv = CVData(
            name="Nguyễn Văn A",
            summary="Kỹ sư phần mềm với 5 năm kinh nghiệm xây dựng API.",
            experience=[
...
class TestBuildCv:
    ...
    def test_build_normalizes_polished_output(self, monkeypatch) -> None:
        ...
        result = build_cv(CVData(name="B"), extractor=extractor)
        assert result.preview_json.skills == ["Python"]
    ...
    def test_build_unifies_mixed_polished_output(self, monkeypatch) -> None:
        ...
        build_cv(CVData(name="B"), language="vi", extractor=extractor)
        assert extractor.polish_calls == 2
        assert extractor.last_polish_language == "vi"
    ...
    def test_build_propagates_extraction_error(self, monkeypatch) -> None:
        ...
        class BrokenExtractor:
            def polish(self, cv, *, language="en", max_attempts=3):
                raise CvExtractionError("invalid structure")

        try:
            build_cv(CVData(name="B"), extractor=BrokenExtractor())
    ...
    def test_build_defaults_to_english(self, monkeypatch) -> None:
        ...
        build_cv(CVData(name="B"), extractor=extractor)
