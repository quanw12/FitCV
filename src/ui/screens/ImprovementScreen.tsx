...
import {
  ...
  scoreLabel,
  sectionLabel,
} from "@/services/improvementReport"
...
export default function ImprovementScreen({
  ...
  const loadReport = useCallback(
    async (regenerate = false) => {
      ...
      try {
        ...
        for (
          let attempt = 0;
          attempt < 30 && ["Pending", "Processing"].includes(current.status);
          attempt += 1
        ) {
          if (!isCurrentRequest()) return
        ...
        if (["Pending", "Processing"].includes(current.status))
          throw new Error(
            "Analysis is taking longer than expected. Please retry shortly.",
          )
        ...
        if (current.status === "Failed") {
          ...
          throw new Error(
            current.errorMessage ?? "AI suggestions could not be generated.",
          )
        }
  ...
  return (
    <div className="improvement-layout">
      <aside
        className="fc-card improvement-sidebar"
        aria-label="Improvement report contents"
      >
        ...
        {response?.overallScore != null && (
          <div
            className="improvement-score"
            aria-label={`Overall score ${response.overallScore}, ${scoreLabel(response.overallScore)}`}
          >
            <span>Overall Match</span>
            <strong>{response.overallScore}%</strong>
      ...
      <main className="improvement-content fc-stagger">
        <header className="improvement-header">
          <div>
            ...
            <p>
              Prioritized recommendations for the selected CV and job
              description.
            </p>
          </div>
          {matchResultId && (
        ...
        <div className="improvement-disclaimer" role="note">
          ...
          <span>
            AI suggestions support your review and do not guarantee a hiring
            outcome. Verify every fact and replace placeholders only with
        ...
        {!matchResultId && (
          <StateCard
            ...
            message="Run a CV and JD analysis, then open its improvement report."
          />
        )}
        {isProcessing && (
          <StateCard
            title="Generating your report"
            message="FitCV is reviewing skill gaps and CV sections. This can take a few moments."
            loading
          />
        )}
        {error && (
        ...
        {response?.stale && (
          <div className="improvement-warning">
            This report may be outdated because the CV or job description
            changed. Regenerate it for current advice.
          </div>
        )}
        ...
        {!isProcessing && !error && report && (
          <>
            <section id="skill-gaps" className="fc-card improvement-section">
              ...
              />
              {skillGaps.length === 0 ? (
                <Empty message="No clear skill gaps were found for this job description." />
              ) : (
            ...
            <section
              id="section-feedback"
              className="fc-card improvement-section"
            >
              <SectionTitle
              ...
              />
              {feedback.length === 0 ? (
                <Empty message="No section-specific feedback is available." />
              ) : (
                feedback.map((item) => (
                  <article key={item.id} className="feedback-item">
                    ...
                    <button
                      ...
                      aria-controls={`feedback-${item.id}`}
                    >
                      <span>
                        {sectionLabel[item.section]} — {item.issue}
                      </span>
                      <span className={priorityBadge(item.priority)}>
                        {item.priority}
                      </span>
            ...
            <section id="rewrites" className="fc-card improvement-section">
              ...
              />
              {rewrites.length === 0 ? (
                <Empty message="No safe rewrite suggestions are available." />
              ) : (
                rewrites.map((item) => (
                  <article key={item.id} className="rewrite-item">
                    <div className="item-heading">
                      <h3>{sectionLabel[item.section]}</h3>
                      <span>{item.framework}</span>
                    </div>
                    <p className="rewrite-issue">
                      <strong>Issue:</strong> {item.issue}
                    </p>
                    <div className="rewrite-grid">
                      <div>
                        <small>Before</small>
                        <p className="rewrite-before">{item.originalText}</p>
                      </div>
                      <div>
                        <small>Suggested rewrite</small>
                        <p className="rewrite-after">{item.suggestedText}</p>
                      </div>
                    </div>
                    <div className="copy-row">
                      ...
                      <button
                        className="fc-btn fc-btn--secondary"
                        onClick={() =>
                          void copyRewrite(item.id, item.suggestedText)
                        }
                        aria-label={`Copy rewrite for ${sectionLabel[item.section]}`}
                      >
                        <Clipboard size={14} /> Copy rewrite
                      </button>
                      <span role="status">{copyMessage[item.id]}</span>
                    </div>
                  </article>
                ))
              )}
              ...
              {quickWins.length > 0 && (
                <>
                  <div className="quick-progress">
                    <div
                      style={{
                        width: `${(completedWins.size / quickWins.length) * 100}%`,
                      }}
                    />
                  </div>
                  <p className="progress-label">
                    {completedWins.size}/{quickWins.length} completed
                  </p>
                </>
              )}
              {quickWins.length === 0 ? (
                <Empty message="No quick wins are available for this report." />
              ) : (
                quickWins.map((item) => {
...
function StateCard({
  ...
  return (
    <div className="fc-card improvement-state" role="status">
      {loading && <div className="state-spinner" />}
      <h2>{title}</h2>
      <p>{message}</p>
      {action}
    </div>
...
function createAbortError(): DOMException {
  return new DOMException(
    "The improvement request was cancelled.",
    "AbortError",
  )
}
